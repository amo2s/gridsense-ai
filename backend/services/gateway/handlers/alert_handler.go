...
type AlertBridgeClient interface {
    ...
    LogIntervention(ctx context.Context, payload *models.InterventionPayload) error // Added for Step 9.2 RL state tracking
}
...
// LogIntervention accepts telemetry from the Next.js UI when an operator manually acts on an alert.
// Step 9.2: This data is routed to PostgreSQL to serve as the reward baseline for Engine D's RL pipeline.
func (h *AlertHandler) LogIntervention(w http.ResponseWriter, r *http.Request) {
    ctx, cancel := context.WithTimeout(r.Context(), 10*time.Second)
    defer cancel()

    ctx, span := alertTracer.Start(ctx, "HTTP.LogIntervention")
    defer span.End()

    var payload models.InterventionPayload
    ...
    start := time.Now()
    // Forward the structured telemetry to the microservice for PostgreSQL insertion
    err = h.client.LogIntervention(ctx, &payload)
    duration := time.Since(start).Seconds()

    if err != nil {
        span.RecordError(err)
        span.SetStatus(codes.Error, err.Error())
        alertLatency.WithLabelValues("log_intervention", "error").Observe(duration)
        h.handleError(w, err)
        return
    }

    alertLatency.WithLabelValues("log_intervention", "success").Observe(duration)
